...
export default function Estimate() {
	...
	const getPlataforms = () => {
		...
		if (questions.length > 2) {
			questions
				...
				.map((option) => newPlataform.push(option.title));

			setPlatforms(newPlataform);
		}
	};
	...
	const sendEstimate = () => {
		setLoading(true)
		axios.get(
			"https://us-central1-material-ui-course-8f498.cloudfunctions.net/sendMail",
			{
				params: {
					name: name,
					email: email,
			...
			<Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="lg" fullScreen={matchesSM} style={{ zIndex: 1302 }} >
				...
				<DialogContent>
					<Grid container justify="space-around" direction={matchesSM ? "column" : "row"} alignItems={matchesSM ? "center" : undefined} >
						...
						<Grid item container direction="column" md={5} alignItems={matchesSM ? "center" : undefined} style={{ maxWidth: "30em" }}>
							<Hidden smDown>
								<Grid item>
